...
public abstract class BitVector implements Comparable<BitVector>
{
     ...
     * return value contains only the bits inside the bounds limited by the bit size.  
     * 
     ...
     * bits that fall beyond the bounds limited by the bit size are ignored. In other words,
    ...
    /**
     * Copies data from the specified bit vector to the current bit vector. If the bit vector
     * is shorter than the current one, the rest high bytes are filled with zeros.
     * If the source bit vector is longer, the result is truncated (the higher part is skipped). 
     * 
    ...
    /***
     * Checks the equality of bit vectors. Bit vectors are considered equal if 
     * their sizes match and they hold equal data (comparison is performed 
    ...
    /**
     * Compares the current bit vector with the specified bit vector. Data is
    ...
    /**
     * Creates a copy of current bit vector. Creates a new bit vector of the same size and 
     * fills it with data from the current bit vector. 
     * 
    ...
    /**
     * Creates a copy of the specified bit vector. Creates a new bit vector of the same size
     * and fills it with data from the current bit vector.
     * 
    ...
    /**
     * Creates a bit vector mapping for the current bit vector. The mapping provides
     * access to a part of the bit vector as if it was a separate bit vector.
     * 
     * @param src Source bit vector.
    ...
    /**
     * Creates a mapping for several bit vectors. The mapping unites the bit vector and
     * allows working with them as if they were a single bit vector.
     * 
    ...
    /**
     * Returns an unmodifiable view of the specified bit vector. An attempt to modify
     * the bit vector will result in the UnsupportedOperationException exception.
     * 
     ...
     * The data size (number of bits) matches the string length.  
     * 
    ...
    /**
     * Creates a bit vector based on textual representation of binary data.
     * The data size is specified by a method parameter. If the length of the string 
     * exceeds the specified size, data is truncated (characters signifying higher bits
     * are ignored). If the string length is less than the data size, the higher bits
     * of the data are filled with zeros.  
     * 
     ...
     * If the bit vector size is less that the long value size (64 bits), the value is truncated
     * (high bits of the value are ignored). If the bit vector size exceeds the long value size,
     * high bits of the bit vector are filled with zeros.
     * 
    ...
    /**
     * Creates a bit vector from a byte array. This method is implemented to 
     * create bit vectors based on data stored in BigInteger. Data is copied
     * starting from the highest byte (it goes to the lowest byte of the bit vector).
     * It is implemented this way because the byte order of data stored in BigInteger
     * is different (opposite) from the byte order in bit vectors. If the bit vector
     * size is greater than the byte array size, the rest of the bit vector (high bytes)
     * is filled with zeros. If the size of the byte array is greater, the highest bytes
    ...
    /**
     * Creates a bit vector based on a integer value. The data size is
     * specified by a method parameter. If the bit vector is less that the integer value
     * size (32 bits), the value is truncated (high bits of the value are ignored). If
     * the bit vector size exceeds the integer value size, high bits of the raw data store
    ...
    {
        if ((index < 0) || (index > size))
            throw new IndexOutOfBoundsException(String.format("Index: %d, Size: %d (included)", index, size));
    }
